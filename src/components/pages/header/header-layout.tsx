...
const useHeaderContext = () => {
  ...
  if (!context) {
    throw new Error("Header compound components must be used within Header.Root");
  }
  return context;
};
    ...
    <header
      className={cn(
        ...
        "border-b border-transparent transition-all duration-300",
        {
          ...
          "dark:supports-[backdrop-filter]:bg-black/40": isScrolled && !isOpenMenu && !isSearchMode,
...
const HeaderLogo = ({ className }: HeaderLogoProps) => {
  ...
  return (
    <FlexibleImage
      src={IMAGES.LOGO}
      alt="logo"
      className={cn("h-[28px] w-[92px] lg:w-[130px] lg:h-10 cursor-pointer", className)}
      onClick={goHome}
    />
  );
};
...
const HeaderMobile = () => {
  ...
  return (
    <div className="lg:hidden">
      {isSearchMode ? (
        <ArrowLeft className="size-6 cursor-pointer" onClick={() => setIsSearchMode(false)} />
      ) : (
        <HeaderLogo />
      )}
    </div>
...
const HeaderNav = ({ className }: HeaderNavProps) => {
  ...
  return (
    <nav className={cn("flex gap-6 lg:gap-20 lg:items-center justify-start", className)}>
      <Button variant="ghost" onClick={goHome} className="flex justify-start">
        Home
      </Button>
      <Button variant="ghost" onClick={goFavorite} className="flex justify-start">
        Favorite
      </Button>
    </nav>
  );
...
const HeaderMobileActions = ({ onSearch, searchValue }: HeaderMobileActionsProps) => {
  const { isOpenMenu, isSearchMode, setOpenMenu, setIsSearchMode } = useHeaderContext();

  if (isOpenMenu) {
    return <X className="w-6 h-6 cursor-pointer" onClick={() => setOpenMenu(false)} />;
  }
  ...
  return (
    <div className={cn("lg:hidden flex gap-6", isSearchMode && "flex-1")}>
      {isSearchMode ? (
        ...
        <>
          <Search className="w-6 h-6 cursor-pointer" onClick={() => setIsSearchMode(true)} />
          <Menu className="w-6 h-6 cursor-pointer" onClick={() => setOpenMenu(true)} />
        </>
      )}
    </div>
  );
...
const HeaderSearch = ({ children, onSearch, searchValue }: HeaderSearchProps) => {
  const { isSearchMode, setIsSearchMode } = useHeaderContext();

  return (
    <SearchMotion.Root isSearchMode={isSearchMode} setIsSearchMode={setIsSearchMode}>
      <SearchMotion.TriggerInput id={"desktop"} handleInputChange={onSearch} value={searchValue} />
      {children}
    </SearchMotion.Root>
  );
};
